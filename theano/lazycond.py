...
class CondPushOut(gof.Optimizer):
    """ Graph optimization for pushin g out ifelse """
    def add_requirements(self, env):
        env.extend(gof.toolbox.ReplaceValidate())

    def candidate(self, node):
        if (not isinstance(node.op, IfElse) and
            any([(x.owner and isinstance(x.owner.op, IfElse)) for x in
                 node.inputs])):
            return True
        return False

    def grab_upper_graph(self, node, extremities):
        clients = set()
        outs    = set()

        node_outs = node.outputs
        while len(node_outs) > 0:
            nw_node_outs = set()
            for o in node_outs:
                if o in extremities:
                    outs.add(o)
                else:
                    for cl,_  in o.clients:
                        if cl !='output':
                            nw_node_outs = nw_node_outs.union(cl.outputs)
                            clients.add(cl)
            #print '..', len(list(clients))
            node_outs = list(nw_node_outs)

        return list(outs), list(clients)

    def apply(self, env):
        extremities = set(env.outputs)
        change = True
        while change:
            change = False
            nw_extremities = set()
            for x in extremities:
                if x.owner and isinstance(x.owner.op, IfElse):
                    nw_extremities = nw_extremities.union(x.owner.inputs)
                    change = True
                else:
                    nw_extremities.add(x)
            extremities = nw_extremities

        extremities = list(extremities)
        topo = gof.graph.io_toposort(env.inputs, extremities)
        ifnodes = [x for x in topo if isinstance(x.op, IfElse)]
        if len(ifnodes) < 1:
            return False
        if_node = ifnodes[-1]
        n_ifnodes = len(ifnodes)
        outs = [x for x in extremities if find_up([x], if_node) ]
        print '>>>>>>>>>>', len(extremities), len(outs)
        print 'ifnodes', len(ifnodes)
        true_ins  = if_node.inputs[1:][:if_node.op.n_outs]
        false_ins = if_node.inputs[1:][if_node.op.n_outs:]
        for out in outs:

            true_eval  = clone([out], replace=dict(zip(if_node.outputs,
                                                            true_ins)))
            false_eval = clone([out],
                               replace=dict(zip(if_node.outputs,
                                                false_ins)))

            nw_out = ifelse(if_node.inputs[0], true_eval, false_eval)
            if type(nw_out) not in (tuple, list):
                nw_out = [nw_out]
            env.replace_all_validate(zip([out], nw_out),
                                     reason='cond_pushout')
...
def cond_lift_single_if(main_node):
    if not (isinstance(main_node.op, acceptable_ops)
        or
            main_node.op.__class__.__name__=='Forward' or
            main_node.op.__class__.__name__=='Backward'):
        return False
    all_inp_nodes = set()
